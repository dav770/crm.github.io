...
class Agents extends Component {
  ...
  displayModal = (
    ...
    <AgentsDetails leState={this.state} idAgent={0}/>
      )
    ...
    const elements = this.state.agents.map((el, index) => {
      ...
      if (this.state.edit) {
        ...
        return (
          ...
          <tr key={el.idAgents}>
            <th scope="row">{el.idAgents}</th>
            <td className={`id-${el.idAgents}`}>{el.name}</td>
            <td className={`id-${el.idAgents}`}>{el.email}</td>
            <td className={`id-${el.idAgents}`}>{el.pwd}</td>
            <td className={`id-${el.idAgents}`}>{el.compagny}</td>
            <td>
              {this.state.edit ? (
                <td>
                  <button
